...
from game.logic import BaghChalGame
from core.redis_client import get_redis

class ConnectionManager:
    def __init__(self):
        # matchId -> set of WebSocket connections
        self.active_connections: Dict[str, Set[WebSocket]] = {}
        # matchId -> game instance
        self.games: Dict[str, BaghChalGame] = {}
        # websocket -> (matchId, userId)
    ...
    async def load_game(self, match_id: str):
        ...
        # Check for board data with both bytes and string keys
        has_data = game_data and (b"board" in game_data or "board" in game_data)
        
        if has_data:
            # Load existing game, handling bytes from Redis
            def get_value(key):
                val = game_data.get(key.encode() if isinstance(key, str) else key) or game_data.get(key)
                return val.decode() if isinstance(val, bytes) else val
            
            goats_placed_data = get_value("goats_placed") or "0"
            
            # Only load if the game has actually started (goats have been placed)
            # This prevents loading stale games from old matches
            ...
            # Otherwise, keep the fresh game instance
    ...
    async def save_game(self, match_id: str):
        ...
        await redis.hset(f"game:{match_id}", mapping={
    ...
    async def broadcast_to_match(self, match_id: str, message: dict):
        ...
        if match_id in self.active_connections:
            # Create a copy to avoid issues if connections change during iteration
