...
from matchmaking.service import add_to_queue, remove_from_queue
...
async def start_matchmaking(user_id: int = Depends(get_current_user_id)):
    ...
    from matchmaking.service import heartbeat_user
...
async def check_match_status(user_id: int = Depends(get_current_user_id)):
    ...
    from matchmaking.service import heartbeat_user, is_user_alive, decode_redis_value, MATCHMAKING_QUEUE
    
    # Only update heartbeat if user is in queue
    ...
    # Check if user has a match assigned
    user_match_raw = await redis.get(f"user_match:{user_id}")
    
    if user_match_raw:
        match_id = user_match_raw.decode() if isinstance(user_match_raw, bytes) else user_match_raw
...
async def leave_match(user_id: int = Depends(get_current_user_id)):
    ...
    from core.redis_client import get_redis
    from matchmaking.service import cleanup_match, decode_redis_value
